...
public:
  ...
  double operator()((std::function<double (double)> funToInt,double a) const
  {
    // Divide the interval [a,b] in subintervals and obtain the quadrature points:
    std::vector<double> weights_ = makeQuadWeights(a,b);
    std::vector<double> qpnts_    = makeQuadPnts(a,b);
  ...
  // Interface class: used for creating the various quadrature
...
public:
  ...
  // a : starting point
  // b : end point
  // n : number of integration subdomain
  double operator () (std::function<double (double)> funToInt,double a, double b) const
  {
    return  qrule_->operator()(funToInt,a,b,n);
  }
...
int main()
{
  ...
  numericalIntegration Int<TrapzQuadrature>{&trpQuad};
