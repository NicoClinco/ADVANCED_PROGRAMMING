...
{
  m.doc() = "pybind-11 data_processor converted";
  
  py::class_<DataProcessor>(m, "DataProcessor")
    .def(py::init([](const std::vector<double> &a) {
      return new DataProcessor(a.data(), a.size());
	}))
    // .def("__add__",[](const DataProcessor &a,
    //		      const DataProcessor &b){return a+b;})
    .def("__getitem__", (const double& (DataProcessor::*)(const unsigned int&) const) &DataProcessor::operator[])
    .def("__setitem__", (double& (DataProcessor::*)(const unsigned int&)) &DataProcessor::operator[])
    .def("__add__",&DataProcessor::operator+)
    .def("__str__",[](const DataProcessor& dp){
      std::stringstream sOut;
      sOut << dp;
      std::string stringa;
      std::getline(sOut,stringa);
      return stringa;
    });
